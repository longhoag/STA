...
# text wrap is under tools -> global options -> code -> soft-wrap R source files.
...
# explicitly stating the arguments lets you put them in any order. So this 
#     prints the number 0 twice, even though the order is different from ?rep.
rep(times=2, x=0) 
rep(x=0, times=2)

# you MUST combine all these different elements with the c() function.
# here we are just copying the data over from the example.
finger_data <- c(rep(0,2),
                 rep(1,3),
                 rep(2,6),
                 rep(3,8),
                 rep(4,5),
...
table(finger_data) # shows the frequency distribution of the data

# assigning the frequency distribution to the variable "freq_dist"
?table # documentation for this one can be confusing.
finger_dist = table(finger_data); finger_dist

# this just does relative-frequency distribution. Note that this is the same as
#     prop.table(table(ex_data)), so you need to get the frequency distribution 
#     first, then the relative frequency after.
?prop.table
prop.table(finger_dist) # relative-frequency distribution

barplot(finger_dist) # how you would do a discrete histogram or barplot of categorical data
barplot(prop.table(finger_dist)) # relative frequency on vertical axis, same shape.
...
?seq
?hist
seq(from=0, to=10, by=2) # sequence of numbers from 0 to 10 incrementing by 2
...
# this is frequency of 6.5. You could store which(data == 6.5) into a variable 
#     above if you wanted, and just do length(that_variable)
f <- length(which(data == 6.5)) 
n <- length(data) # size of the data set, number of observations.
f / n # this is relative frequency, so 10% of the data is equal to 6.5
...
hist(data, breaks = seq(min(data), max(data), length.out=8), right=F)
barplot(data)
...
f <- function(n){
  ...
  #     if we assign the function call to a variable, such as new_data <- f(20)
  #     then new_data will hold the value of what gets returned.
  return(data)
}
f(20)
new_data <- f(200)
